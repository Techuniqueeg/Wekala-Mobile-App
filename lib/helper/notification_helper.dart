...
import 'package:wekala_user/common/widgets/demo_reset_dialog_widget.dart';
import 'package:wekala_user/common/widgets/taxi_make_payment_bottomsheet.dart';
...
import 'package:wekala_user/features/rental_module/rental_order/screens/taxi_order_details_screen.dart';
...
import 'package:wekala_user/features/ride_share_module/ride_home/widgets/driver_request_dialog.dart';
...
import 'package:wekala_user/features/notification/widgets/notifiation_popup_dialog_widget.dart';
...
class NotificationHelper {
  static Future<void> initialize(
    ...
    FirebaseMessaging.onMessage.listen((RemoteMessage message) async {
      if (kDebugMode) {
        print(
          "onMessage notification: ${message.notification?.title} / ${message.notification?.body}",
        );
      ...
      if (kDebugMode) {
        print(
          'pusher Disconnected: $pusherDisConnected [${Get.find<SplashController>().pusherConnectionStatus == null} || ${Get.find<SplashController>().pusherConnectionStatus} || ${Get.find<SplashController>().configModel!.webSocketStatus == false}]',
        );
      }

      /// For Ride Share
      if (message.data['action'] == 'customer_driver_on_the_way' &&
          pusherDisConnected) {
      ...
      } else if (message.data['action'] == 'customer_trip_started' &&
      ...
      } else if ((message.data['action'] == 'customer_trip_resumed' ||
              message.data['action'] == 'customer_trip_paused') &&
          message.data['type'] == 'ride_request') {
      ...
      } else if (message.data['action'] == 'customer_trip_canceled' &&
          message.data['type'] == 'ride_request' &&
          pusherDisConnected) {
        Get.find<SafetyAlertController>().cancelDriverNeedSafetyStream();
        ...
        Get.find<SafetyAlertController>().getSafetyAlertDetails(
          ...
          Get.currentRoute.contains('/RidePaymentScreen')) {
        Get.offAll(
          () => RideOrderCompleteScreen(
            tripId: message.data['ride_request_id'],
            fromNotification: true,
  ...
  static Future<void> rideNotificationRouteCheck(
    Map<String, dynamic> data, {
    ...
    if (data['type'] == 'ride_request') {
      ...
      if (Get.find<SplashController>().module?.moduleType !=
          AppConstants.ride) {
    ...
    if (data['action'] == 'customer_driver_on_the_way') {
      ...
      notificationToRouteNavigate(data['ride_request_id'], formSplash);
    } else if ((data['action'] == 'customer_trip_resumed' ||
            data['action'] == 'customer_trip_paused') &&
        data['type'] == 'ride_request') {
      notificationToRouteNavigate(data['ride_request_id'], formSplash);
  ...
  static void _executeRideRequestAction(
    Map<String, dynamic> data,
    bool formSplash,
  ) {
    if (data['action'] == 'customer_driver_on_the_way') {
      ...
      Get.find<RideController>().getFinalFare(data['ride_request_id']).then((
        value,
      ) {
        if (value.statusCode == 200) {
          Get.find<RideController>().updateRideCurrentState(
