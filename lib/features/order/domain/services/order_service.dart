...
class OrderService implements OrderServiceInterface {
  ...
  @override
  Future<PaginatedOrderModel?> getOrderList(
    OrderListType type,
    int offset, {
    bool fromDashboard = false,
    int? moduleId,
  }) async {
    return await orderRepositoryInterface.getOrderList(
      type,
      offset,
  ...
  @override
  Future<List<LastOrderModel>?> getLastOrders({
    required bool isHome,
    int? storeId,
  }) async {
    return await orderRepositoryInterface.getLastOrders(
      isHome: isHome,
      storeId: storeId,
    );
  }
  ...
  @override
  Future<MonthlyOrderModel?> getMonthlyOrderList({
    required int offset,
    String? moduleType,
  }) async {
    return await orderRepositoryInterface.getMonthlyOrderList(
      offset: offset,
  ...
  Future<Response> trackOrder(
    ...
    return await orderRepositoryInterface.trackOrder(
      orderID,
      guestId,
  ...
  Future<bool> cancelOrder({
    ...
    return await orderRepositoryInterface.cancelOrder(
      orderID: orderID,
      reason: reason,
      guestId: guestId,
      isParcel: isParcel,
  ...
  Future<bool> switchToWalletPayment(String? orderID) async {
    bool isSuccess = false;
    Response response = await orderRepositoryInterface.switchToWalletPayment(
      orderID,
    );
    if (response.statusCode == 200) {
      isSuccess = true;
    ...
    if (canRedirect) {
      ...
      if (forOrder) {
        if (isSuccess) {
          ...
          Get.offNamed(
            RouteHelper.getDigitalPaymentFailedScreen(
              orderID,
              createAccount: createAccount,
            ),
          );
  ...
  void _proSubscriptionRedirect(bool isSuccess, bool isRenew) {
    ...
    Get.find<ProController>().showPlanSubscribeState(
      isRenew,
      redirect: true,
